...
# ponytail: `kimi -p` only accepts one argv prompt; split the workflow step if
# Linux's per-argument limit becomes a real constraint.
...
MODEL_PROVIDERS = {"codex", "claude", "kimi", "openrouter", "xai"}
...
def _claude_env(env: dict[str, str], model: str, model_provider: str | None = None) -> dict[str, str]:
    ...
    if provider in {"openrouter", "kimi"}:
        ...
        routed_model = OPENROUTER_MODEL_ALIASES.get(model, model) if provider == "openrouter" else model
